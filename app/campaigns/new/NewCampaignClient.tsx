...
export default function NewCampaignClient({ userId }: { userId: string }) {
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "0 auto", padding: "48px 40px 80px" }}>
      <div style={{ marginBottom: "48px" }}>
        ...
        <h1 style={{ fontSize: "36px", fontWeight: "normal", color: "var(--color-text)", margin: 0 }}>New Campaign</h1>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
        <div>
          ...
          <input name="name" value={form.name} onChange={handleChange} placeholder="e.g. Curse of Strahd" style={inputStyle}
            onFocus={e => e.currentTarget.style.borderColor = "var(--color-gold)"}
            onBlur={e => e.currentTarget.style.borderColor = "var(--color-border)"} />
        </div>
        <div>
          <label style={labelStyle}>Setting</label>
          <input name="setting" value={form.setting} onChange={handleChange} placeholder="e.g. Forgotten Realms, Barovia, Custom" style={inputStyle}
            onFocus={e => e.currentTarget.style.borderColor = "var(--color-gold)"}
            onBlur={e => e.currentTarget.style.borderColor = "var(--color-border)"} />
        </div>
        <div>
          <label style={labelStyle}>Description</label>
          <textarea name="description" value={form.description} onChange={handleChange} placeholder="A brief overview of your campaign..." rows={4}
        ...
        {error && <p style={{ color: "#c06060", fontSize: "13px", margin: 0 }}>⚠ {error}</p>}
        ...
        <div style={{ display: "flex", gap: "16px", paddingTop: "8px" }}>
          ...
          <button onClick={() => router.back()} style={{
            ...
            padding: "14px 36px", fontSize: "12px", letterSpacing: "0.2em", textTransform: "uppercase",
